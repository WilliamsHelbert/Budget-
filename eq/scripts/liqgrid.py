...
SETS = [("2024 nov-dec","db2024"), ("2025","db2025"), ("2026","vendata")]
def go(folder, **extra):
...
def st(g,lbl):
    if len(g)<2: return "%-26s n %3d"%(lbl,len(g))
    r=g.sort_values('id').R.values; e=np.cumsum(r)
    return ("%-26s n %3d | R %+7.2f | pr.h %+5.2f | DD %+7.2f | vind%% %4.1f | TP %2d SL %2d BE %2d | t %+5.2f"
...
VAR = [("GAMMEL prev5 cap40", dict(be_ref='prev', max_be={'ES':8,'NQ':40})),
       ("liq piv0 cap40",     dict(be_src='liq', liq_piv=0, max_be={'ES':8,'NQ':40})),
       ("liq piv0 cap75",     dict(be_src='liq', liq_piv=0, max_be={'ES':15,'NQ':75})),
       ("liq piv0 ingen cap", dict(be_src='liq', liq_piv=0)),
       ("liq piv1 cap75",     dict(be_src='liq', liq_piv=1, max_be={'ES':15,'NQ':75})),
       ("liq piv1 ingen cap", dict(be_src='liq', liq_piv=1))]
for lbl, folder in SETS:
    print("="*104); print("###", lbl)
    for vl, kw in VAR:
        n = go(folder, **kw); s = n[n.side=='SHORT']
        print(st(s, vl+" short"))
